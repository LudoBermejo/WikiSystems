...
# Google Chrome Installer/Uninstaller for CentOS 6 or 7
...
# - 32-bit or 64-bit CentOS 6.5 or later (you will be asked to
#   upgrade the OS and reboot if you're running 6.4 or earlier) or
#   any 64-bit CentOS 7 version (including pre-releases)
...
# These CentOS 6 RPMs and their (many!) deps that aren't already installed:
# redhat-lsb, wget, xdg-utils, GConf2, libXSCrnSaver, libX11,
# gcc, glibc-devel, nss, rpm-build and rpmdevtools.
# These CentOS 7 RPMs and their (many!) deps that aren't already installed:
...
# CentOS 6 only: 5 RPM packages from Fedora 15 if not already fully downloaded.
# CentOS 6 only: 1 RPM package from Fedora 17 if not already fully downloaded.
...
# - Minimum CentOS 6 release supported is now 6.5, which has been out for over
#   6 months at the time of writing. This means libX11 and nss should be
...
#   CentOS 6, so that's been added and the chrome-deps RPM has been bumped
#   to version 1.20.
# - Added nss to the list of possible CentOS 6 RPMs that are installed (thanks
...
check_7_repos()
# See if there's any repos we haven't created in /etc/yum.repos.d.
...
# dependencies from the latest pre-release CentOS 7.
# This code will be removed once CentOS 7 final is out.
{
   our_repo="/etc/yum.repos.d/chrome-deps-updates.repo"
   # Note that Google Chrome RPM installation adds google-chrome.repo,
   # so we exclude that from the repo count
   case `ls /etc/yum.repos.d/*.repo 2>/dev/null | grep -v /google-chrome.repo | wc -l` in
   0) cat <<@EOF >$our_repo
[chrome-deps-updates]
name=CentOS 7 - Chrome Dependencies Updates
baseurl=http://buildlogs.centos.org/centos/7/os/x86_64-latest/
gpgcheck=0
@EOF
   message "$our_repo created" ;;
   1) a=1 ;; # Either our repo or a user-created one, so leave alone
   *) if [ -s $our_repo ]
      then
...
# Check for RHEL 6 or 7 family
{
   ...
   *_6.*) centos=6 ;;
   ...
   if [ $centos -eq 7 ]
   then
      # CentOS 7 doesn't need chrome-deps RPM or build tools,
...
# Initialisation complete, so run the code
{
   ...
   then
      ...
      yum_install prompt redhat-lsb wget xdg-utils GConf2 libXScrnSaver libX11 nss $rpm_build_packages
